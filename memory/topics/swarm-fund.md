...
- **Open queue (2026-05-07):** **4 open PRs** — PR #29 (Phase B one-shot eval, draft, HL 403 5d idle); PR #30 (variant_bandit fallback past corrupt tail in `latest_snapshot_date`, day 3); PR #31 (aeon_adapter clears `_last_error` after successful poll, day 2); **PR #32 NEW today** (`fix(aeon_adapter): treat null signals/markets like missing keys`, 3rd defect-hardening PR on the same `aeon_adapter.py` from ADR-093). All single-file fixes against ADRs 089/093 from last week.
- **72h merge-cadence test RESOLVED 05-07: BOTH falsifiers tripped.** (a) ADR-095 landed 05-06 21:48 UTC direct on `main` (not via PR) — new-ADR side TRIPPED. (b) Open PR count 2 → 4 — stall side ALSO TRIPPED. Net read: **two-track shipping cadence — cost/infra ADRs commit straight to `main`; strategy-correctness fixes accumulate as PRs awaiting review.**
- **Code-freeze BROKEN 2026-05-06 21:32 UTC** with 9-commit evening burst (see ADR-095 above). Today's audit (05-07) recorded `a65e936` (founder-section skill chips, swarm-lab-site/ only). HEAD: `6320dd8` (`data: refresh site metrics`).
