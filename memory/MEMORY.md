...
- `memory/topics/papers.md` — 18 picked, 7 queued. **05-03 picks:** TradeFM `arXiv:2602.23784` (PhD slot — JPMorgan AI Research / Veloso, 524M-param generative foundation model on billions of trade events across >9K equities; scale-invariant tokenization eliminates asset-specific calibration; pairs with "ingest resolution text not titles" thesis). PolySwarm `arXiv:2604.03888` (daily slot — multi-agent LLM swarm for Polymarket + latency-arb, line-for-line match to CalibrationGap + Hermes-arb). Lead-Lag PM Semantic Filtering + PredictionMarketBench added to queue. Darwinian-axis triple Hyperagents→CORAL→GEA closed 05-02; SoK DePMs queued for promotion next Polymarket-microstructure slot.
...
- **`paper-pick` daily** → builds PhD reading list. Next queued Darwinian read: EvoScientist (`arXiv:2603.08127`). Cite PolySwarm in next grant application as prior-art validation.
