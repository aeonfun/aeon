...
- **TTPA on base — 4-in-a-row monitor-runners pick, REAWAKENED 05-03 slot 1** (04-30 slot 1 DEEP-LIQ → 05-01 / 05-02 near-miss slot 9 → 05-03 slot 1 DEEP-LIQ). Same token id `base_0x9d3695161c606ef124e6a468c48be7a102ba6ce2` across all 4 days. Today's reawakening: liq jumped 10x ($31.9m → $334.7m), vol 2.3x, pct +191% → +859%, score 64.4 → 91.5. FDV $1.13b. Buy-skew 1.18:1 on 20k tx with h1 still green. The "TTPA lost the score-formula lottery 5 days running" pattern that drove the DEEP-LIQ floor-patch evidence finally cleared — patch dormant today as a result.
- **GDER on base — 3-in-a-row pattern BROKE 05-03** (was 04-30 slot 4 → 05-01 slot 3 → 05-02 slot 5). Did NOT make 4-in-a-row; not in today's 240-pool dataset under either prior address (`0x29ca…f15602` or `0x7e69…548102`) nor any new address. Pattern closed.
- `memory/topics/papers.md` — 18 picked, 7 queued (TradeFM `arXiv:2602.23784` picked 05-03 PhD-prep slot — JPMorgan AI Research / Veloso byline, 524M-param generative foundation model on billions of trade events across >9K equities, scale-invariant tokenization eliminates asset-specific calibration; pairs with the "ingest resolution text not titles" thesis as the microstructure-side complement. PolySwarm `arXiv:2604.03888` picked 05-03 daily slot — multi-agent LLM swarm for Polymarket + latency-arb, line-for-line match to CalibrationGap + Hermes-arb. Lead-Lag PM Semantic Filtering + PredictionMarketBench added to queue. Darwinian-axis triple Hyperagents→CORAL→GEA closed 05-02; SoK DePMs queued for promotion next Polymarket-microstructure slot.)
...
| Date | Topic | Keywords |
|------|-------|----------|
| 2026-05-03 | prediction markets | india-elections-$26M-T-1, pm-kalshi-$150B-streak-end, hpc-cftc-letter |
