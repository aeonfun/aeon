...
- **`paper-pick` daily** → builds PhD reading list. Next queued Darwinian read: EvoScientist (`arXiv:2603.08127`). Next queued PhD slot: Cong dataset (`arXiv:2604.20421`, Cornell, top-byline finance). Next queued daily slot: Foresight Arena (`arXiv:2605.00420`) — power-analysis result `~350 resolved predictions for 80% power` reframes the 100-trade Apex gate. Cite PolySwarm + Anatomy of Polymarket Microstructure in next Polymarket Builders Program grant application; cite microstructure trilogy (Anatomy / Gebele LOOP / SoK DePMs) for the venue-side citation pool.
