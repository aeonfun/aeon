...
- `memory/topics/papers.md` — 20 picked, 11 queued. **05-04 PhD-prep pick (third invocation, var=Stanford-PhD-prep):** Cong dataset `arXiv:2604.20421` (Jia / Zhou / Zhang / **Lin William Cong** / Li / Sun, Apr 22 2026, cs.LG) — first continuously-maintained Polymarket lifecycle dataset; **770k market records × 943M fill records × ~2M oracle events** Oct 2020 → Mar 2026; three canonical layers (market metadata + fill-level trades + oracle-resolution events) joined via identifier resolution + on-chain recovery + incremental updates; NBA-calibration + CPI-reconstruction case studies; project page polymonitor.club. Cong (Cornell SC Johnson) is the highest-bar finance byline paper-pick has surfaced this month — Stanford committee citation anchor. Direct map for swarm-fund-mvp's planned "ingest resolution text not titles" ADR rebuild + CalibrationGap quant scanner. Completes venue-side citation triangle with Anatomy + Foresight Arena. Promoted from queue per morning daily-slot log explicit flag. **05-04 second daily pick:** Foresight Arena `arXiv:2605.00420` (Nechepurenko/Shuvalov, May 1 2026, cs.MA) — first permissionless on-chain Brier benchmark; ~350 predictions for 80% power at α=0.02 means CalibrationGap's 100-trade Apex gate is power-underspecified. **05-04 morning daily pick:** Anatomy of Polymarket Microstructure `arXiv:2604.24366` (Dubach, Apr 27 2026, q-fin.TR / cs.GT) — 30B WebSocket events × 52 days joined on-chain; load-bearing methodology finding that public order-book trade-direction inference is only ~59% accurate (must come from on-chain `OrderFilled`). **05-03 picks:** TradeFM `arXiv:2602.23784` (PhD — JPMorgan / Veloso, 524M-param FM on billions of trade events; pairs with "ingest resolution text not titles" thesis). PolySwarm `arXiv:2604.03888` (daily — multi-agent LLM swarm for Polymarket + latency-arb, line-for-line match to CalibrationGap + Hermes-arb). Queue promotions ready: Cong dataset `arXiv:2604.20421` next PhD slot (top-byline finance, Cornell); Foresight Arena `arXiv:2605.00420` next daily (350-prediction-for-80%-power reframes 100-trade Apex gate). Darwinian-axis triple Hyperagents→CORAL→GEA closed 05-02.
...
## Recent Articles
- 2026-05-04 — *Polymarket Got Bengal Right and Tamil Nadu Wildly Wrong on the Same Day* — Counting day for five Indian states. WB market settled BJP 51 / TMC 48.9 at T-1 ($6.5M vol, ~$2M US wallets), validated by 92.47%-turnout BJP-200+ blockbuster — late flip held because two priced venues (Polymarket + Phalodi) tracked observable phase-by-phase turnout. TN market opened DMK 87.5 / TVK 6.95 / AIADMK 6.65 ($22M vol, ₹209 crore — biggest single-state contest); reality TVK 110 / AIADMK 59 / DMK 48, Stalin lost Kolathur by 7,731 votes to TVK first-time candidate. **Calibration thesis:** single-venue confidence is a feature to fade. Cross-venue convergence works only when both venues price the same evidence; when both venues share the same continuity prior (Dravidian duopoly unbroken since 1967), agreement is shared blind spot, not signal. Reinforces "ingest resolution text not titles" upgrade — TN binary said "DMK win," structural question was "can a debut party clear the duopoly threshold." Same ticker, different problem.
- 2026-05-04 — *Aeon Stopped Adding Capabilities This Week. It Started Building the Launch.* — repo-article on `aaronjmars/aeon`. 9 of 9 feature PRs merged Apr 27–May 4 serve the meta-loop: 7 observability/manifest skills (#145 SHOWCASE, #146 heartbeat-token-pulse, #147 pr-triage, #149 smithery-manifest, #152 fork-cohort, #153 operator-scorecard, #157 skill-freshness, commit `32c77d7`), 2 launch-text generators (#148 thread-formatter, #151 show-hn-draft). Zero new market/research/content-ingest skills shipped. show-hn-draft itself flags timing: "~12 days from 300-star milestone at ~4/day momentum" — operator at 270 stars, mid-May launch implied. Counter-evidence: `c95478c` README-badge removal is launch-cosmetic; PR #156 (reply-maker XAI prefetch) is feature-skill repair, not meta; no public launch announcement yet. Falsifier: if next 7 days' feature PRs revert to >50% market/research/content (non-meta), this was a one-week artifact. Re-check 2026-05-11. Pivot reading: defensible moat for a Claude-Code-on-cron framework isn't more skills — it's the meta-loop that watches its own runs survive contact with users.
