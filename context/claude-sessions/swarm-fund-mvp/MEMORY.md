...
- [api.rswarm.ai must be hosted off the founder laptop](project_api_rswarm_host_decision.md) — 2026-05-01 decision: host the FastAPI (Cloudflare tunnel preferred), reject mailto-shim fallback. Reason = laptop SPOF, not just funnel.
...
- [feat/fs-adoption branch held](project_fs_adoption_branch_held.md) — 23 commits at `/Users/stew/scaria/swarm-fund-mvp-fs-adoption` (branch `feat/fs-adoption`). All five workstreams shipped: A1 NAV recon + A4 lint ext + B schemas/handoff + A3 position audit + C MCP/ADJ/CryptoPanic. 61/61 fs_adoption tests pass; dashboard builds; 3 new API routes; phantom-peak replay catches 2026-04-25 incident. **Held unmerged** because parallel main work claims ADR-103 + ADR-104 for spec-driven strategy authoring; my branch's 14 commits and ADR stub also claim ADR-103. Resume: confirm parallel ADRs landed → renumber to ADR-105 in single tip-commit (also writes Task 23 final Rationale body per plan) → final regression → merge.
