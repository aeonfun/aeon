#!/bin/bash
set -e
BASE=/home/runner/work/aeon/aeon/secdigest-tmp
...
for SEV in critical high; do
  if [ -n "$GITHUB_TOKEN" ]; then
    curl -sf --max-time 30 "https://api.github.com/advisories?type=reviewed&severity=${SEV}&published=${SINCE48}.." \
  ...
  echo "advisories_${SEV}: $(jq 'length' "$BASE/advisories_${SEV}.json" 2>/dev/null || echo err)"
done
