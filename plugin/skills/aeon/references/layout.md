...
`ls` prints `SKILL / ON / SCHEDULE / PACK / DESCRIPTION` and a footer — `84 skills · 1 enabled`. First run installs the CLI runtime (tsx + yaml, ~12MB, one-time); the noise is expected.
