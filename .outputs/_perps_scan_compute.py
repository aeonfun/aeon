...
for a, m in metrics.items():
    ...
    repeat_days = (yrep + 1) if (yreg == regime) else 1
    results[a] = {
...
for reg in order:
    for a, r in results.items():
        ...
        print(f"{reg:16s} {a:9s} T{r['tier']} | 24h={g('pct_24h')} 7d={g('pct_7d')} 4h={g('pct_4h')} "
              ...
              f"fnow={g('funding_now',4)} favg={g('funding_7d_avg',4)} fΔ={g('funding_delta',4)} tls={g('top_ls_now')} dls={g('top_ls_delta_7d')} "
...
with open(".outputs/_perps_scan_results.json", "w") as fh:
