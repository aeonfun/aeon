...
function yesterdayRegimeFor(asset) {
  ...
  const prev = priorByAsset.get(asset);
  return prev ? prev.yesterday_regime ?? null : null;
}
...
const regime_empty_notes = {
  ...
  'CATALYST-BREAKOUT':
    "XAN cleared three of four gates with pct_24h +32.88%, OI +319.81% 24h, vol_ratio 56.7x, " +
  ...
  CAPITULATION:
    ...
    "VVV pct_24h -1.95% on OI -4.44% 24h sits the right side of funding < 0 with -0.0171% but the drawdown gate blocks by 8.05pp",
};
...
const watch = [
  ...
  },
  {
    ...
    transition_read:
      ...
      "OI flipped positive on the 24h read at +0.83% from -0.83% at 08:14Z on the +40.38% 7d build. " +
...
for (const a of Object.keys(metrics)) {
  ...
  tail.push({
    ...
    metrics: {
      ...
      pct_7d: r2(m.pct_7d),
      pct_4h: r2(m.pct_4h),
      range_7d: m.range_7d_pct === null || m.range_7d_pct === undefined ? null : `${r2(m.range_7d_pct)}%`,
    ...
    yesterday_regime: yesterdayRegimeFor(a),
...
const assessed = tail.length;
const neutralCount = tail.filter((t) => t.regime === 'NEUTRAL').length;
const watchAssets = new Set(watch.map((w) => w.asset));
const neutralSummary = `Neutral · ${neutralCount - watchAssets.size} other assets · see artifact tail for full data`;
...
console.log(
  `wrote .outputs/perps-scan.data.json — assessed=${assessed} neutral=${neutralCount} watch=${watch.length} regime_changes=null (re-run)`,
);
